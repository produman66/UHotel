...
import com.example.uhotel.imageslider.ViewPagerScroller
import com.example.uhotel.imageslider.adapters.ViewPagerAdapter
...
import com.example.uhotel.imageslider.interfaces.ItemClickListener
import com.example.uhotel.imageslider.interfaces.TouchListener
...
class ImageSlider @JvmOverloads constructor(
    ...
    private var unselectedDot1 = 0
    private var unselectedDot2 = 0
    private var unselectedDot3= 0
    private var unselectedDot4 = 0
    ...
    init {
        ...
        unselectedDot1 = typedArray.getResourceId(R.styleable.ImageSlider_iss_unselected_dot, R.drawable.default_unselected_dot1)
        unselectedDot2 = typedArray.getResourceId(R.styleable.ImageSlider_iss_unselected_dot, R.drawable.default_unselected_dot2)
        unselectedDot3 = typedArray.getResourceId(R.styleable.ImageSlider_iss_unselected_dot, R.drawable.default_unselected_dot3)
        unselectedDot4 = typedArray.getResourceId(R.styleable.ImageSlider_iss_unselected_dot, R.drawable.default_unselected_dot4)
    ...
    private fun setupDots(size: Int) {
        ...
        for (i in 0 until size) {
            ...
            dots!![i]!!.setImageDrawable(ContextCompat.getDrawable(context, unselectedDot1))
        ...
        viewPager!!.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageScrolled(
                ...
                // Этот метод вызывается, когда страница прокручивается
            }

            override fun onPageSelected(position: Int) {
                currentPage = position

                // Установка изображения для текущей точки
                dots!![position]!!.setImageDrawable(ContextCompat.getDrawable(context, selectedDot))

                // Установка изображений для соседних точек в зависимости от расстояния
                for (i in 1..4) { // Можете увеличить 3 до нужного вам максимального расстояния
                    val prevIndex = position - i
                    val nextIndex = position + i

                    if (prevIndex >= 0) {
                        dots!![prevIndex]!!.setImageDrawable(
                            ContextCompat.getDrawable(context, getUnselectedDotDrawable(i))
                        )
                    }

                    if (nextIndex < size) {
                        dots!![nextIndex]!!.setImageDrawable(
                            ContextCompat.getDrawable(context, getUnselectedDotDrawable(i))
                        )
                    }
                }

                // Вызов слушателя изменения элемента, если установлен
                if (itemChangeListener != null) itemChangeListener!!.onItemChanged(position)
            }

            override fun onPageScrollStateChanged(state: Int) {
                // Этот метод вызывается, когда состояние прокрутки изменяется
            }
        })
    }
    private fun getUnselectedDotDrawable(distance: Int): Int {
